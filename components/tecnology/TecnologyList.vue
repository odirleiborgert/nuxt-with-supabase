...
const { loading, tecnologies, categoryName, openForm, deleteTecnology, fetchTecnologies } = tecnologiesStore
const filter = useTecnologyFilter()

const filteredTecnologies = computed(() => {
    if (!filter.value) return tecnologies.value
    return tecnologies.value.filter(t => t.category_id === filter.value)
})
